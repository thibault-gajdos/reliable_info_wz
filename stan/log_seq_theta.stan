...
data {
  ...
  real l_proba[N,T_max, I]; // sequences of logit probabilities
...
parameters {
  ...
  vector[3] mu_pr; // alpha, beta, theta means
  vector<lower=0>[I-1] sigma_w; // sequential weights var 
  vector<lower=0>[3] sigma; // alpha, beta, theta
...
transformed parameters {
  ...
  for (n in 1:N){
    ...
    theta[n] = Phi_approx(mu_pr[3] + sigma[3]*theta_raw[n]) * 5;
    for (i in 1:5){
    ...
    for (t in 1:Tsubj[n]) {
      ...
      for (i in 1:I) {
        ...
        if (color[n, t, i] == 1) { // color = blue
          ...
          evidence[1] += weight*(alpha[n] * l_proba[n, t, i] + beta[n]);
        }
        if (color[n, t, i] == 2) { // color = red
          // evidence[2] += weight*(alpha[n] * log(proba[n, t, i] / (1 - proba[n, t, i])) + beta[n]);
          evidence[2] += weight*(alpha[n] * l_proba[n, t, i] + beta[n]);
        }
      }
      // softmax choice
      val = softmax(theta[n]*evidence);
